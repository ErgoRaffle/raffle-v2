{
  ...
  //   1: RaffleLicense
  // Context:
  //   C0[Coll[Int]]: WinnersPercentList (in raffle creation tx)
  //
  ...
  val raffleLicense = fromBase64("RAFFLE_LICENSE")
  ...
  if (OUTPUTS(0).tokens(0)._1 == ownerNft) {
    ...
    if(outputService.tokens(1)._2 == SELF.tokens(1)._2 + 1L){
      ...
      // New raffle creation
      // [Service(Self), UserBox] --> [Service, TicketRepo, InactiveRaffle, Change]
      val winnersPercent = getVar[Coll[Int]].get(0)
      val winnerPercentsSum = winnersPercent.fold(0L, {(x: Long, y: Long) => x + y})
      val winnerCount = winnersPercent.size
      ...
      sigmaProp(allOf(Coll(
        ...
        // R4: [CharityPercentage, ServiceFeePercent, TicketPrice, Goal, Deadline, TotalSoldTicket, WinnerCount, CreationFee]
        // R5: [ServiceAddress, CharityAddress, Name, Description, Pictures(optional)]
        // R6: [TicketId, WinnerListHash]
        ...
        inactiveRaffle.R4[Coll[Long]].get(6) == winnerCount,
        inactiveRaffle.R4[Coll[Long]].get(7) == creationFee,
        inactiveRaffle.R5[Coll[Coll[Byte]]].get.size >= 4,
        inactiveRaffle.R5[Coll[Coll[Byte]]].get(0) == serviceAddress,
        inactiveRaffle.R6[Coll[Coll[Byte]]].get(0) == SELF.id, // Storing TicketId to match with TicketRepo
        inactiveRaffle.R6[Coll[Coll[Byte]]].get(1) == blake2b256(winnersPercent),
        inactiveRaffle.value == (winnerCount * (fee + minBoxValue)) + ((2 * fee) + minBoxValue) + creationFee,
