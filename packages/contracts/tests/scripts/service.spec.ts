...
const createRaffleServiceTest = (winnersCount: bigint = 1n) => {
  ...
  serviceBox.setContextExtension({ 
    ...
    1: SColl(SColl(SByte), [
      Array.from(blake2b256(Buffer.from(rosen.address.toString(), 'hex'))),
      Array.from(blake2b256(Buffer.from(creator.address.toString(), 'hex')))
    ])
   });
...
describe('Service', () => {
  ...
  const raffleServiceBy5WinnersTest = createRaffleServiceTest(5n);
