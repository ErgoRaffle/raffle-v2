...
import { serviceBoxInit } from './serviceBoxInit';
...
const main = async () => {
  ...
  if (configs.init.enabeled) {
    logger.info('Service box initialization is enabled, starting init flow');
    await serviceBoxInit(logger.child('ServiceBoxInit'));
  } else {
